...
def load_data(source_list, target_list, batch_size, num_workers, task = 'single'):
    ...
    train_t_dataset = office31('train_t', train_transform(),target_transform=Onehot(), target_train=True)
    ...
    return query_dataloader, train_s_dataloader, train_t_dataloader, retrieval_dataloader#, query_data, retrieval_data


class office31(Dataset):
    def __init__(self, mode, transform=None, target_transform=None, target_train=False):
    ...
    def __getitem__(self, index):
        ...
        if self.target_train:
            img_aug_1 = self.aug(img)
            img_aug_2 = self.aug(img)
            return img_aug_1, img_aug_2, self.target_transform(self.targets[index]), index
        else:
            return img_, self.target_transform(self.targets[index]), index
    ...
    def init(source_list, target_list, task):
        ...
        with open(source_list, 'r') as f:
            for line in f.readlines():
                source_data.append(line.split()[0].replace(\
                    '/data/office/domain_adaptation_images','/data/user/da_datasets/office/office31'))
        ...
        if task == 'single':
            ...
            unique_labels, counts = np.unique(target_label, return_counts=True)
            query_per_class = np.ceil(counts * query_ratio).astype(int)

            query_index = np.array([], dtype=int)

            for i in range(num_classes):
                class_indices = np.where(target_label == i)[0]
                perm_index = np.random.permutation(class_indices)
                query_index = np.concatenate([query_index, perm_index[:query_per_class[i]]])

            retrieval_t_index = np.setdiff1d(np.arange(target_data.shape[0]), query_index)
            np.save('query_index.npy', query_index)
            np.save('retrieval_index.npy', retrieval_t_index)


            #query_index = np.load('query_index.npy')
            #retrieval_t_index = np.load('retrieval_index.npy')
            ...
            office31.TRAIN_T_DATA = target_data[retrieval_t_index]
            office31.TRAIN_T_TARGETS = target_label[retrieval_t_index]

            office31.RETRIEVAL_DATA = target_data[retrieval_t_index]
            office31.RETRIEVAL_TARGETS = target_label[retrieval_t_index]
